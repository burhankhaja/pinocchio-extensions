...
pub trait Token2022PausableExtension {
    ...
    fn token_2022_try_pause_multisig(
        &mut self,
        target: Target,
        mint: &Pubkey,
    ...
    fn token_2022_try_resume_multisig(
        &mut self,
        target: Target,
        mint: &Pubkey,
        multisig_authority: &Pubkey,
    ...
    fn token_2022_query_pausable_config(
        &self,
        target: Target,
        mint: &Pubkey,
    ) -> TestResult<PausableConfig>;
}

impl Token2022PausableExtension for App {
    fn token_2022_try_initialize_pausable(
        &mut self,
        target: Target,
        sender: AppUser,
    ...
    fn token_2022_query_pausable_config(
        ...
        match target {
            Target::Spl => {
                ...
                // get the PausableConfig extension
            ...
            Target::Proxy => {
                use pinocchio_token_2022::extension::pausable::state::PausableConfig as PinocchioPausableConfig;

                let state =
                    PinocchioPausableConfig::from_bytes(data).map_err(TestError::from_raw_error)?;

                Ok(PausableConfig {
                    authority: to_optional_non_zero_pubkey(Some(state.authority())),
                    paused: state.is_paused().into(),
                })
            }
        }
    }
