use {
    crate::helpers::suite::{
        ...
        types::{
            addr_to_sol_pubkey, pin_pubkey_to_addr, AppUser,
...
impl Token2022CpiGuardExtension for App {
    fn token_2022_try_enable_cpi_guard(
        ...
        let signers = &[&owner.keypair()];
        
        // create authority signers for the instruction
        let authority_signers: Vec<_> = multi_signers
            .iter()
        ...
        match target {
            Target::Spl => {
                ...
                let account_with_extensions =
                    StateWithExtensions::<Account>::unpack(data).map_err(TestError::from_raw_error)?;
